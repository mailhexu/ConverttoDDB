...
Ang2Bohr=1/0.5291772108; Bohr2Ang=0.5291772108; ev2Ha=1/27.2113845; Ha2eV=27.2113845; e_Cb=1.602176487e-19;
...
############################################################################
# Read qpoints.yaml file (from Phonopy) and build the DDB for atomic displacement perturb.
            ...
            if (len(dynmat_data) >0):
                ...
                for ipert1 in range(natoms):
                    for ipert2 in range(natoms):
                        dm1=dm[ipert1*3:(ipert1+1)*3,ipert2*3:(ipert2+1)*3]
                        print ipert1,ipert2,ipert1*3,(ipert1+1)*3,ipert2*3,(ipert2+1)*3,dm1
                        dm1=np.matmul(rprimd,np.matmul(dm1,rprimd.transpose()))
                        for idir1 in range(3):
                            for idir2 in range(3):
                                dynmat[iq,idir1,ipert1,idir2,ipert2]=dm1[idir1,idir2]
                                phi[iq,idir1,ipert1,idir2,ipert2]=dm1[idir1,idir2]*np.sqrt(massatom[ipert1]*massatom[ipert2]);
                                DDB[iq,idir1,ipert1,idir2,ipert2]=phi[iq,idir1,ipert1,idir2,ipert2]*ev2Ha/(Ang2Bohr*Ang2Bohr)
...
# first get the type number of each atom:
    ...
    for ipert1 in range(natoms):# ipert1 is atom position deriv
        ...
        dm1=np.matmul(rprimd,np.matmul(bec[ipert1,:,:]-dij[:,:]*zion[type_atom[ipert1]], gprimd))
        #dm1=np.matmul(rprimd,np.matmul(bec[ipert1,:,:]-dij[:,:]*zion[type_atom[ipert1]], rprim.transpose()))
        for idir1 in range(3):
            for idir2 in range(3):
                DDB[0,idir1,ipert1,idir2,ipert2] = dm1[idir1,idir2]*2*np.pi + 0.0j
                DDB[0,idir2,ipert2,idir1,ipert1] = dm1[idir1,idir2]*2*np.pi + 0.0j
                ...
                print " %3d %3d %3d %3d  %E  %E"% \
                  ...
                  np.real(DDB[0,idir2,ipert2,idir1,ipert1]),np.imag(DDB[0,idir2,ipert2,idir1,ipert1]))
...
#               pure E-field pertubations <--> epsilon^infty                
    ...
    factor=volume/(acell[idir1]*acell[idir2])
    ipert1 = natoms + 1
    ipert2 = natoms + 1
    ...
    for idir1 in range(3):
        for idir2 in range(3):
            DDB[0,idir1,ipert1,idir2,ipert2] = dm1[idir1,idir2]*np.pi*volume + 0.0j 
...
def readSYSTEM():
    ...
    if os.path.isfile(sfile) and os.access(sfile, os.R_OK):
       ...
       for line in data:
           ...
           if ("acell" in line):
               d=line.split()
               if (len(d)>4):
                   acell[:]=np.array(map(float,d[1:4]))*Ang2Bohr
                   print "entre"
               else:
                   acell[:]=np.array(map(float,d[1:4]))
               print "acell in a.u. = ",acell
       ...
       while True:
           ...
           if ("rprimvasp" in line):
               ...
               if (np.prod(acell) != 1.0):
                   for i in range(3):
                       rprimvasp[i,:]=rprimvasp[i,:]*acell[i]*Bohr2Ang
               for i in range(3):
                       rprimd[:,i]=rprimvasp[i,:]
               for i in range(3):
                   acell[i]=np.sqrt(np.dot(rprimvasp[i,:],rprimvasp[i,:]))*Ang2Bohr
                   rprim[:,i]=rprimvasp[i,:]/acell[i]
               print "rprim = ",rprim
               print "rprimd = ",rprimd
               print "rprimvasp = ",rprimvasp
           ...
           if ("rprim" in line and "vasp" not in line and "rprimd" not in line):
               ...
               for i in range(3):
                   rprimvasp[i,:]=rprim[:,i]*acell[i]*Bohr2Ang
               print "rprim = ",rprim
               print "rprimd = ",rprimd
               print "rprimvasp = ",rprimvasp
           if ("rprimd" in line):
    ...
    print "***************** typat = ",typat
    volume=np.dot(rprimd[:,0],np.cross(rprimd[:,1],rprimd[:,2]))
    print "volume =",volume
                   ...
                   for ipert1 in range(natoms+2):
                      for idir1 in range(3):
                         if (ipert1==natoms or ipert2==natoms):
                            ...
                            ddbfile.write(" %3d %3d %3d %3d % .14E  %.14E\n"%(idir1+1,ipert1+1,idir2+1,ipert2+1, \
                                         np.real(DDB[0,idir1,ipert1,idir2,ipert2]),np.imag(DDB[0,idir1,ipert1,idir2,ipert2])))
                         elif (ipert2 == natoms+1 and ipert1 != natoms and ipert1 != natoms+1):  # mixed E/R pert <--> Z*
                            ddbfile.write(" %3d %3d %3d %3d % .14E  %.14E\n"%(idir1+1,ipert1+1,idir2+1,ipert2+1, \
                                         np.real(DDB[0,idir1,ipert1,idir2,ipert2]),np.imag(DDB[0,idir1,ipert1,idir2,ipert2])))
